...
Time::Time(unsigned int hour, unsigned int minute, unsigned int second) {
    if (hour < 24) {
        ...
        throw std::out_of_range("Hour value greater than 24");
    }
    if (minute < 60) {
        this->minute = minute;
    } else {
        throw std::out_of_range("Minute value greater than 60");
    }
    if (second < 60) {
        this->second = second;
    } else {
        throw std::out_of_range("Second value greater than 60");
    }
}
